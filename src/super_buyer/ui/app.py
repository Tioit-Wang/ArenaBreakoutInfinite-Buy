...
from super_buyer.config import ConfigPaths, ensure_default_config, load_config, save_config
...
from super_buyer.ui.tabs.profit import ProfitTab
from super_buyer.ui.widgets import LightTipManager
from super_buyer.ui.tabs.tasks import SingleFastBuyTab
...
class App(tk.Tk):
    def __init__(self) -> None:
        ...
        try:
            ...
            if not isinstance(paths_cfg, dict):
                paths_cfg = {}
                self.cfg["paths"] = paths_cfg
            paths_cfg["output_dir"] = str(self.paths.output_dir)
        except Exception:
            pass
        # 首次运行：确保 goods.json 存在（从包内默认数据复制）
        try:
            self._ensure_default_goods(self.paths.root / "goods.json")
        except Exception:
            pass
    ...
    def _resolve_data_root(self) -> Path:
        ...
        - 冻结态（PyInstaller 目录分发）：`<exe_dir>/data`
        - 开发态（源码运行）：`<当前工作目录>/data`
        """
        try:
    ...
    def _ensure_default_goods(self, path: Path) -> None:
        ...
        - 若不存在，则优先从包内 `super_buyer.resources.defaults/goods.json` 复制；
        - 若包内无默认数据，则写入一个空列表 `[]` 作为占位。
        """
        path = Path(path)
        if path.exists():
            return
        try:
            with pkg_resources.as_file(
                pkg_resources.files("super_buyer.resources.defaults") / "goods.json"
            ) as src:
                try:
