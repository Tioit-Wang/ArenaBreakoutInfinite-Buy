...
class ScreenOps:
    ...
    def __init__(self, cfg: Dict[str, Any], step_delay: float = 0.01) -> None:
        ...
        try:  # pyautogui is optional at import time
            ...
            raise RuntimeError("缺少 pyautogui 或其依赖，请安装 pyautogui + opencv-python。") from e
    ...
    def locate(self, tpl_key: str, region: Optional[Tuple[int, int, int, int]] = None, timeout: float = 0.0) -> Optional[Tuple[int, int, int, int]]:
    ...
    def click_center(self, box: Tuple[int, int, int, int], clicks: int = 1, interval: float = 0.05) -> None:
        l, t, w, h = box
        x = int(l + w / 2)
        y = int(t + h / 2)
        try:
...
class Buyer:
    ...
    def __init__(self, cfg: Dict[str, Any], screen: ScreenOps, on_log: Callable[[str], None]) -> None:
        self.cfg = cfg
        self.screen = screen
        self.on_log = on_log
        ...
        self._cur_engine = str(ca.get("ocr_engine") or cfg.get("avg_price_area", {}).get("ocr_engine") or "umi")
    ...
    def _ensure_ready(self, startup_timeout_sec: float) -> bool:
        ...
        self.on_log(f"【{_now_label()}】【全局】【-】：开始启动，超时点 {time.strftime('%H:%M:%S', time.localtime(end))}")
        while time.time() < end:
            ...
            if self.screen.locate("btn_market", timeout=0.2) is not None:
                return True
            _sleep(0.2)
        self.on_log(f"【{_now_label()}】【全局】【-】：启动失败，未见市场按钮，任务终止。")
        return False
    ...
    def _open_goods_detail(self, goods: Goods, *, prefer_cached: bool = False) -> Tuple[bool, int, str]:
        ...
        if goods.image_path and os.path.exists(goods.image_path):
            try:
                box = self._pg_locate_image(goods.image_path, confidence=0.80, timeout=2.0)
                if box is not None:
    ...
    def _pg_locate_image(self, path: str, confidence: float, timeout: float = 0.0) -> Optional[Tuple[int, int, int, int]]:
        end = time.time() + max(0.0, float(timeout or 0.0))
        while True:
            try:
    ...
    # ----- price read -----
    def _read_currency_avg_price(self, item_disp: str, purchased_str: str) -> Optional[int]:
        ...
        if abs(sc - 1.0) > 1e-3:
            try:
                ch, cw = crop.shape[:2]
                crop = cv2.resize(crop, (max(1, int(cw * sc)), max(1, int(ch * sc))), interpolation=cv2.INTER_CUBIC)
            except Exception:
                pass
        ...
        if not eng:
            eng = str((self.cfg.get("avg_price_area", {}) or {}).get("ocr_engine", "umi") or "umi").lower()
        txt = ""
        try:
            if eng in ("tesseract", "tess", "ts"):
                allow = str((self.cfg.get("avg_price_area", {}) or {}).get("ocr_allowlist", "0123456789KM"))
                need = "KMkm"
                allow_ex = allow + "".join(ch for ch in need if ch not in allow)
                cfg = f"--oem 3 --psm 6 -c tessedit_char_whitelist={allow_ex}"
                txt = read_text(oimg, engine="tesseract", grayscale=True, tess_config=cfg)
            else:
            ...
            if eng in ("umi", "umi-ocr", "umiocr"):
                self._log(item_disp, purchased_str, f"Umi-OCR 失败: {e} | eng={eng} roi=({x1},{y1},{x2},{y2}) scale={sc:.2f} score={_s:.2f}")
                raise FatalOcrError(str(e))
            else:
                self._log(item_disp, purchased_str, f"CurrencyOCR失败: {e} | eng={eng} roi=({x1},{y1},{x2},{y2}) scale={sc:.2f} score={_s:.2f}")
                return None
        val = _parse_price_text(txt or "")
        if val is None or val <= 0:
            self._log(item_disp, purchased_str, f"CurrencyOCR解析失败：'{(txt or '').strip()[:64]}' | eng={eng} roi=({x1},{y1},{x2},{y2}) scale={sc:.2f} score={_s:.2f}")
            return None
        # Log key parameters for debugging
        self._log(item_disp, purchased_str, f"CurrencyOCR成功 val={val} eng={eng} roi=({x1},{y1},{x2},{y2}) scale={sc:.2f} score={_s:.2f}")
        return int(val)

    def _read_avg_unit_price(self, goods: Goods, item_disp: str, purchased_str: str) -> Optional[int]:
        # Anchor at Buy button (same as 平均单价预览)
        t_btn = time.perf_counter()
        # Prefer cached buy button; fallback to a short locate
        buy_box = self._detail_ui_cache.get("btn_buy") or self.screen.locate("btn_buy", timeout=0.3)
        btn_ms = int((time.perf_counter() - t_btn) * 1000.0)
        if buy_box is None:
            self._log(item_disp, purchased_str, f"未匹配到‘购买按钮’模板，无法定位平均价格区域 match={btn_ms}ms")
            return None
        ...
        try:
            if eng in ("tesseract", "tess", "ts"):
                ...
                txt = read_text((bin_top or img_top), engine="tesseract", grayscale=True, tess_config=cfg)
            ...
            if eng in ("umi", "umi-ocr", "umiocr"):
                self._log(item_disp, purchased_str, f"Umi-OCR 失败: {e} | eng={eng} roi=({x_left},{y_top},{x_left+width},{y_top+mid_h}) scale={sc:.2f} btn_ms={btn_ms} ocr_ms={ocr_ms if ocr_ms>=0 else '-'}")
                raise FatalOcrError(str(e))
            else:
                self._log(item_disp, purchased_str, f"AvgOCR失败: {e} | eng={eng} roi=({x_left},{y_top},{x_left+width},{y_top+mid_h}) scale={sc:.2f} btn_ms={btn_ms} ocr_ms={ocr_ms if ocr_ms>=0 else '-'}")
                return None
        val = _parse_price_text(txt or "")
        if val is None or val <= 0:
            self._log(item_disp, purchased_str, f"AvgOCR解析失败：'{(txt or '').strip()[:64]}' | eng={eng} roi=({x_left},{y_top},{x_left+width},{y_top+mid_h}) scale={sc:.2f} btn_ms={btn_ms} ocr_ms={ocr_ms if ocr_ms>=0 else '-'}")
            return None
        self._log(item_disp, purchased_str, f"AvgOCR成功 val={val} eng={eng} roi=({x_left},{y_top},{x_left+width},{y_top+mid_h}) scale={sc:.2f} btn_ms={btn_ms} ocr_ms={ocr_ms}")
        return int(val)
        ...
        *,
        ...
        if not skip_search:
            ...
            if not self._search(query):
                ...
                return 0, True
            self._log(item_disp, purchased_str, f"匹配 btn_market → input_search({query}) → btn_search 成功")
        # Open detail by goods image (prefer cached pos when skipping search)
        ok_open, cost_ms, src = self._open_goods_detail(goods, prefer_cached=bool(skip_search))
        if not ok_open:
            if src == "cached":
                self._log(item_disp, purchased_str, f"缓存坐标无效，未进入详情 open={cost_ms}ms")
            else:
                # Template路径失败仍保留匹配耗时语义
                self._log(item_disp, purchased_str, f"未匹配到商品图片模板 match={cost_ms}ms")
            return 0, True
        if src == "cached":
            self._log(item_disp, purchased_str, f"使用缓存坐标进入详情 open={cost_ms}ms")
        else:
            # 统一指标：模板路径成功也输出 open=...（包含匹配+验证）
            self._log(item_disp, purchased_str, f"匹配 goods.image_path 模板并进入详情 open={cost_ms}ms")
        ...
        if unit_price is None or unit_price <= 0:
            ...
            c = self._detail_ui_cache.get("btn_close") or self.screen.locate("btn_close", timeout=0.3)
            if c is not None:
        ...
        remain = (max(0, target_total - purchased_so_far) if target_total > 0 else max_per_order)
        ...
        if restock > 0 and unit_price <= restock:
            if (goods.big_category or "").strip() == "弹药":
                ...
                b = self._detail_ui_cache.get("btn_max") or self.screen.locate("btn_max", timeout=0.3)
                if b is not None:
        ...
        b = self._detail_ui_cache.get("btn_buy") or self.screen.locate("btn_buy", timeout=0.3)
        if b is None:
            c = self._detail_ui_cache.get("btn_close") or self.screen.locate("btn_close", timeout=0.3)
            if c is not None:
                self.screen.click_center(c)
        ...
        if got == "ok":
            # Close detail
            c = self._detail_ui_cache.get("btn_close") or self.screen.locate("btn_close", timeout=0.5)
            if c is not None:
                self.screen.click_center(c)
            self._log(item_disp, f"{purchased_so_far + q}/{target_total}", f"购买成功(+{q})，已关闭详情")
            return q, True
        elif got == "fail":
            c = self._detail_ui_cache.get("btn_close") or self.screen.locate("btn_close", timeout=0.3)
            if c is not None:
...
class TaskRunner:
    ...
    def __init__(
        ...
        *,
        ...
        # Derived helpers
        step_delay = float(((self.tasks_data.get("step_delays") or {}).get("default", 0.01)) or 0.01)
    ...
    def _ensure_ready(self) -> bool:
        startup_timeout = float(((self.cfg.get("game") or {}).get("startup_timeout_sec", 120)) or 120)
        return self.buyer._ensure_ready(startup_timeout)
    ...
    def _run_round_robin(self, tasks: List[Dict[str, Any]]) -> None:
        ...
        while not self._stop.is_set():
            ...
            self._relay_log(f"【{_now_label()}】【{item_disp}】【{purchased}/{target}】：开始片段，时长 {duration_min} 分钟")
            ...
            try:
                ...
                self._relay_log(f"【{_now_label()}】【全局】【-】：Umi-OCR 失败（片段初始化），终止任务：{e}")
                self._stop.set()
                break
            ...
            while not self._stop.is_set() and time.time() < seg_end:
                ...
                # Execute one attempt
                try:
                    got, _cont = self.buyer.execute_once(goods, t, purchased, skip_search=True)
                except FatalOcrError as e:
                    self._relay_log(f"【{_now_label()}】【全局】【-】：Umi-OCR 失败（循环中），终止任务：{e}")
                    self._stop.set()
                    break
                if got > 0:
                    purchased += int(got)
            ...
            t["status"] = "idle"
            self._relay_log(f"【{_now_label()}】【{item_disp}】【{purchased}/{target}】：片段结束，累计 {elapsed} ms")
            idx += 1
    ...
    def _run_time_window(self, tasks: List[Dict[str, Any]]) -> None:
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                self._relay_log(f"【{_now_label()}】【全局】【-】：Umi-OCR 失败（进入窗口），终止任务：{e}")
                self._stop.set()
                break
            ...
            while not self._stop.is_set():
                ...
                try:
                    ...
                    self._relay_log(f"【{_now_label()}】【全局】【-】：Umi-OCR 失败（窗口循环），终止任务：{e}")
                    self._stop.set()
                    break
                if got > 0:
                    purchased += int(got)
            ...
            self._relay_log(f"【{_now_label()}】【{t.get('item_name','')}】【{purchased}/{target}】：退出时间窗口")
    ...
    def _validate_tasks(self, tasks: List[Dict[str, Any]]) -> None:
        ...
        for t in tasks:
            ...
            if not gid:
                ok = False
                self._relay_log(f"【{_now_label()}】【{t.get('item_name','')}】【-】：缺少 item_id，任务无效")
            g = self.goods_map.get(gid) if gid else None
            if not g:
                ok = False
                self._relay_log(f"【{_now_label()}】【{t.get('item_name','')}】【-】：goods.json 未找到该 item_id")
            else:
                if not (g.search_name or "").strip():
                    ok = False
                    self._relay_log(f"【{_now_label()}】【{t.get('item_name','')}】【-】：goods.search_name 为空，任务无效")
                if not (g.image_path and os.path.exists(g.image_path)):
                    ok = False
                    self._relay_log(f"【{_now_label()}】【{t.get('item_name','')}】【-】：goods.image_path 不存在，任务无效")
            t["_valid"] = bool(ok)
